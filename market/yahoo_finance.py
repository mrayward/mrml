...
def load_yahoo_quote(ticker, begindate, enddate, info = 'quote'):
    ...
    tb = begindate.timestamp()
    te = enddate.timestamp()
    ...
    for line in lines:
        ...
        if first:
            ...
            if line != '':
                arr = line.replace('null', 'nan').split(',')
                idx[k] = arr[0]
                dta[k, :] = np.array(arr[1:], dtype=float)
            k += 1

    df = pd.DataFrame(data=np.array(dta), columns=hdr, index=pd.to_datetime(idx))
    return df